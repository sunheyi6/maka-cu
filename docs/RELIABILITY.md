...
- Linux runtime 依赖已登录桌面用户 session 的 `XDG_RUNTIME_DIR`、`DBUS_SESSION_BUS_ADDRESS` 和 display 环境；纯 SSH tty 默认不能直接访问 AT-SPI GUI tree。
...
5. 排查 Linux runtime 时，先确认目标命令是否运行在桌面用户 session 中，再用 `open-computer-use call list_apps` 和 `open-computer-use snapshot <app>` 区分 session/env 问题与 AT-SPI tree/action 问题。
