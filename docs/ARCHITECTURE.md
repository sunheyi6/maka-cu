...
- overlay 的 visual style 不再自己从官方 app bundle 裁 `SoftwareCursor` 小图；runtime 现在直接复用 `CursorMotion` 同款的程序化 pointer/fog glyph 校准，命中点 anchor 固定在 `126x126` 画布里的同一组 tip-offset 上，避免实验室和主 MCP runtime 各自维护一套 cursor 轮廓。
