...
extension HostProtocolServer {
    func handleDispatchPoint(id: Int, params: HostDispatchPointParams) {
        ...
        // Keep the method as a stable refusal endpoint for older hosts. Point
        // mutation is not an executor capability: do not resolve or consume the
        // snapshot, inspect the target, select a path, or post an event.
        emit(
            id: id,
            toolCallId: params.toolCallId,
            dispatchFailure: HostDomainError(.unsupportedAction),
            tier: .coordinateBackground
        )
    }
